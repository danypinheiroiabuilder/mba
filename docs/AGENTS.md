...
A regra geral de prova (alegação sem evidência = hipótese; busca truncada; 2ª verificação em achados críticos) está no `~/.claude/CLAUDE.md`. Aqui só o que é local: use `scripts/verificar-contraste.mjs` quando for contraste deste app; rode comando real neste repo antes de declarar correção.
